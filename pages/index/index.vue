<template>
	<view class="content">
			...
			<view class="">
				...
				<view class="" v-else-if="item.name=='可选功能'">
					<view class="flex-between li_106" v-for="o_item in Optional" :key='o_item.key'
						...
						<!-- 不返还 -->
						<switch :checked=" formdata[ o_item.key]" @change="change($event,0)"
		...
		<van-action-sheet :show="priceShow" :actions=" priceAction" @close="priceShow=false" @select="change($event,4)"
		...
		methods: {
			...
			changeDateTime1(e) {
				...
				new Promise((resolve, reject) => {
					...
					// 当前选中时间戳
					let nowTimeStamp = this.nowTimeStamp
