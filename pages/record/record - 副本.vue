	...
	<view class="container">
		...
		<van-tabs swipeable @change='tabchange' animated>
			...
			<van-tab v-for='(item,index) in datalist' :key='item.id' :title="item.type">
				<view class='item' v-if="formdata.name==0">
					<view class="">
						<view class="">
					...
					<view class="flex-around">
						...
						<button type="default" plain open-type='share'><text class="iconfont icon-fenxiang" @click="openPop('share')">分享</text></button>
						<button type="default" plain><text class="iconfont icon-delete2" @click="remove(item.id,index)">删除</text></button>
					</view>
				</view>
			...
			getList(data){
				...
				if(data.type==1){
					...
					this.$apis.SELL_LIST(data).then(res=>{
						...
						Object.assign(this.datalist[1],{content:res})
						uni.hideLoading()
					})
				}else{
					// 
